...
class CitationRepository extends ServiceEntityRepository
{
    ...
    public function findRandomCitation()
    {
        $allCitations = $this->findAll();

        if (empty($allCitations)) {
            return null; // No citations in the database
        }

        $randomIndex = array_rand($allCitations);

        return $allCitations[$randomIndex];
    }
